...
PY=${PY:-python}
DESIGN_ROOT=${DESIGN_ROOT:-benchmarks/Titan23_benchmark/}
RESULT_ROOT=${RESULT_ROOT:-./results/}
TAG=${TAG:-HySpecPro_th20}
UB=${UB:-0.2}
KWAY=${KWAY:-2}
N_CMA_ITE=${N_CMA_ITE:-5}
DEVICES=(${DEVICES_OVERRIDE:-cuda:0 cuda:1 cuda:2 cuda:3})

DESIGNS=(
  sparcT1_core neuron stereo_vision des90 SLAM_spheric cholesky_mc segmentation
  bitonic_mesh dart openCV stap_qrd minres cholesky_bdti denoise sparcT2_core
  gsm_switch mes_noc LU230 LU_Network sparcT1_chip2 directrf bitcoin_miner
)
